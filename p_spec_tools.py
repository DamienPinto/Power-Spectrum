...
def make_power_spectrum(fourier_universe):
	...
	#The biggest distance from the origin is the one at the furthest corner so the biggest k would be the norma at the max length in each dimension.
	...
	#universe/matter distribution but squared so posting the square root should return just (approximately) the standard deviation
...
def get_k_axis(L, N, dx, dim):
	...
	the range of possible wavenumbers k that could be detected given a space of length L and N regularly spaced sampling intervals.
	...
	Output:
		-k(np.array)  : Array specifiying the range of possibly sampled wavenumbers, partitionned with appropriate step size given
					    N, L, and dx.
		-k_min(float) : Smallest wavenumber possibly sampled given specifics of space (basically 0, because you can always just have
					    a space filled with the same value everywhere regardless of its size).
	...
	k_step	   = (k_max - k_min)/(N/2) #N/2 because np.fft.fftn returns a space of the same size as the input space 
									   #and the origin of the fourier space is at (N/2, N/2), where every cell away from the origin
									   #represents a specific wavenumber vector who's magnitude is found in [k_min, k_max] that 
									   #was sampled.
	
	'''
	Given that the fourier space created by np.fft.fftn is the same size as the distribution its given, but has it's origin 
	in the middle of the space at the (N/2, N/2,... N/2) cell, the furthest distance that a point can be from the origin in fourier space
	is sqrt((N/2)^2 + (N/2)^2 + ...) = sqrt(num_dimensions)*N/2 cells away. So the furthest distance from the origin.
	'''
...
def get_2Ddist_w_k_spike_pspec(L, N, k_spike_loc=-1, amplitude=200, gauss=1, sigma=0.15):
	...
	'''
	Function that makes a 2D distribution based on a power spectrum with a spike of a specified ampitude at a specified value 
	of k (at a specific wavenumber). 
	This can be a pure spike (0 everywhere except for specified location) or a gaussian with a specified standard deviation (sigma).

	Input:
		-L(float)		   : Total length of space in the space's units.
		-N(int)			   : Number of steps/divisions spanning one direction of the space (assumes all directions have same number
							 of steps)
		-k_spike_loc(float): Location in 1/(units of L) indicatig the value at which to create a spike in the power spectrum.
							 The valid range for k-values given L and N will be computed and k_spike_loc must be within this range
	...
	Output:
		-dist_2D (np.ndarray): 2D array that was created from the power spectrum specified by the input parameters and made to have a
							   very similar power spectrum. Due to random elements in population/creation of space, it's power
							   will not be EXACTLY like the one used to creat the space, but it will have very similar 
...
def plot_2Ddist_n_p_spec(k, dist, p_spec, p_spec_errs):
	...
	NOTE: Could add string input indicating units of values found in distribution and units of partitioning of space to add proper
		  units to power spectrum axes.
	'''
	...
	# kept that^ line because sometimes the rounding involved in getting the length of the power spectrum causes a difference 
	# in length by 1 between k and p_spec
	if len(k) != len(p_spec):
